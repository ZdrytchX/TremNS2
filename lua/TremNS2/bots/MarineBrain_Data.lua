...
--test to buy other weapons
function(bot, brain)

    local name = "grabFlameThrower"

    local marine = bot:GetPlayer()
    local haveflamer = marine:GetWeapon( Flamethrower.kMapNam ) ~= nil
    local haveShotgun = marine:GetWeapon( Shotgun.kMapName ) ~= nil
    local flamers = GetEntitiesWithinRangeAreVisible( "FlameThrower", marine:GetOrigin(), 20, true )

    flamers = FilterArray( flamers, function(ent) return ent:GetParent() == nil end )
    local bestDist, bestflamer = GetNearestFiltered(marine:GetOrigin(), flamers)

    local weight = 0.0
    if not haveflamer and bestflamer ~= nil then
        weight = EvalLPF( bestDist, {
                {0.0  , 10.0} ,
                {3.0  , 10.0} ,
                {5.0  , 1.0}  ,
    ...
    if haveShotgun ~= nil then weight = 0 end
    ...
    return { name = name, weight = weight,
            perform = function(move)
                if bestflamer ~= nil then
                    PerformMove( marine:GetOrigin(), bestflamer:GetOrigin(), bot, brain, move )
                    if bestDist < 1.0 then
                        move.commands = AddMoveCommand( move.commands, Move.Drop )
                    end
                end
...
--end removed
    ...
    function(bot, brain)
        ...
        local weaponTechs = { [kTechId.FlamethrowerTech] = kTechId.Flamethrower, [kTechId.GrenadeLauncherTech] = kTechId.GrenadeLauncher, [kTechId.ShotgunTech] = kTechId.Shotgun, [kTechId.Welder] = kTechId.Welder }
...
local function GetAttackUrgency(bot, mem)
    ...
    {
        [kMinimapBlipType.TunnelEntrance] = numOthers >= 1 and 0.2 or 0.95,
        [kMinimapBlipType.Crag] = numOthers >= 2           and 0.2 or 0.9, -- kind of a special case
...
function CreateMarineBrainSenses()
    ...
    s:Add("nearestPower", function(db)
            ...
            local dist, power = GetMinTableEntry( powers,
                function(power)
                    if power:GetIsBuilt() --[[or power:GetCanBeWelded()]] then --TODO fix if power can be repaired
