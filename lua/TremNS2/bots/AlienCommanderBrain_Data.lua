...
kAlienComBrainActions =
{
    ...
    CreateUpgradeStructureAction( kTechId.ShadowStep       , 2.0+math.random() ) ,
    ...
    function(bot, brain)
        ...
        local cysts = position and GetEntitiesForTeamWithinRange("Cyst", com:GetTeamNumber(), position, 4) or {}
        local cyst = #cysts > 0 and cysts[1]
        if not sdb:Get("resPointToTake") and position and (not cyst or not cyst:GetIsActuallyConnected()) then
            weight = 9
        end
