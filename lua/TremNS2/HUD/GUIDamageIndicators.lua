...
GUIDamageIndicators.kIndicatorSize = 512--128
...
function GUIDamageIndicators:Update(deltaTime)
    ...
    for i, indicatorItem in ipairs(self.indicatorItems) do
        ...
        local currentAngle = damageIndicators[currentIndex + 1] --random
...
function GUIDamageIndicators:OnTakeDamage(position, rotation, hitType)
    ...
    hitEffect:SetRotation(Vector(0, 0, rotation + 90 - math.random() * rotation / 2))
